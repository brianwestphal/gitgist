...
- **FR-14 Local provider** (transport now shared with `openai-api` via `providers/openaiCompatible.ts`, FR-34) — Shipped. `--provider local` → OpenAI-compatible endpoint (Ollama/LM Studio) via `providers/local.ts:createLocalProvider`; `--endpoint`/`GITGIST_LOCAL_ENDPOINT`, `--model`/`GITGIST_LOCAL_MODEL`. Opt-in only (not in `AUTO_ORDER`).
...
- **FR-19 Gemini CLI provider** — Shipped, **legacy**. `--provider gemini` → `providers/gemini.ts` (`gemini --skip-trust -p "<prompt>"`, `-m <model>`; `--skip-trust` because headless runs otherwise refuse with exit 55 in any untrusted workspace — GG-65 — and trust is a separate axis from tool approval, which stays default); no key, in `AUTO_ORDER` **after `antigravity`**. Gemini CLI stopped serving Google AI Pro/Ultra and free-tier requests on **2026-06-18** (`IneligibleTierError`); superseded by FR-33 but kept for Code Assist Standard/Enterprise licensees. Spec: `docs/5-providers.md`.
...
- **FR-21 `--model` for CLI agents** — Shipped. `providers/cli.ts` `CliProviderSpec.runArgs` accepts a `model`-function form so `codex`/`gemini`/`opencode` place `-m <model>` and `antigravity` places `--model <model>` correctly.
...
- **FR-33 Antigravity CLI provider** — Shipped. `--provider antigravity` → `providers/antigravity.ts` (`agy -p "<prompt>"` print mode, `input: 'arg'`, `--model <model>` before `-p`); no key, reuses the `agy` Google sign-in. Google's **replacement for the Gemini CLI**. Placed **before `gemini`** in `AUTO_ORDER` because `gemini`'s `--version` probe still succeeds for tiers its backend stopped serving on 2026-06-18, so ordering — not probing — is what keeps `auto` off a dead backend. Model ids come from `agy models` and contain spaces/parens (`Gemini 3.6 Flash (High)`). Verified end-to-end. Spec: `docs/5-providers.md` (GG-62).
- **FR-34 OpenAI API backend** — Shipped. `--provider openai-api` → `providers/openaiApi.ts`, calling chat-completions over **plain `fetch` — no vendor SDK, no new runtime dependency** (GG-32 answered REST over the `openai` package because `local.ts` already spoke the protocol; `tests/conventions.test.ts` pins deps to `@anthropic-ai/sdk` + `apple-fm`). Transport extracted to `providers/openaiCompatible.ts`, now shared with `local` (FR-14) and preserving per-backend error wording. `isAvailable()` = `OPENAI_API_KEY` present, **no network call** (like `anthropic-api`), so a bad key fails at generation time. In `AUTO_ORDER` after the CLI backends and `anthropic-api`, before `apple` — a paid API never outranks an existing sign-in. Model: `--model` → config → `GITGIST_OPENAI_MODEL` → `gpt-5`; `OPENAI_BASE_URL` retargets (Azure/proxy). **Partly verified live:** an invalid key yields a real `401` (not `404`/`400`), confirming URL/headers/body; the **success path and default model id remain unverified** — no OpenAI key on this machine. Spec: `docs/13-openai-compatible.md` (GG-32).
- **FR-35 `--cwd` reaches the AI backend** — Shipped. `GenerateRequest.cwd` carries the resolved repo dir; `providers/cli.ts` passes it to `spawn`. Before this, `--cwd <repo>` redirected git only and the CLI child ran in gitgist's own cwd — `--provider codex` from outside a repo failed on a valid one (codex refuses outside a trusted git dir), and an agent CLI in the wrong dir reads *that* dir's `AGENTS.md`/`CLAUDE.md`. Undefined = inherit, as before; non-spawning providers ignore it; the FR-23 fallback gets the same cwd. Fixes GG-67.
- **FR-36 Honest transport failures (OpenAI-compatible backends)** — Shipped. A timeout reports as a timeout (`isAbort` → `timeoutHint`), never as "not reachable"; other failures name the cause (`ECONNREFUSED`, …) and attach `cause`. `local` gets its own **10-min** `LOCAL_TIMEOUT_MS` vs the shared 120 s — a 12B model measured 87–109 s, so the old budget was intermittently exceeded and misreported as a dead server. `providers/openaiCompatible.ts` + `providers/local.ts`. Fixes GG-64; the original hypothesis (flaky 3 s probe) was wrong.
- **FR-37 Hash guard is structural** — Shipped. `prompt.ts:stripUnrequestedHashes`, applied in `releaseNotes.ts` after `cleanModelOutput` when `--link-commits` is off: removes trailing hash citations a model volunteered (`apple` did — GG-63). Matches **only hashes actually supplied for the range**, not a hex regex, so `(3x faster)` / `(#123)` / `(defaced)` survive. Bare + comma-separated + Markdown-link forms; trailing only; no-op under `--link-commits` and when the range has no commits; idempotent. Spec: `docs/10-attribution.md`.
