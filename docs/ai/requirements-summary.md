...
- **FR-9 CLI flags** — Shipped. `cliArgs.ts` (`--no-ai/--provider/--model/--endpoint/--fallback-provider/--fallback-endpoint/--fallback-model/--max-tokens/--no-diff/--max-diff-chars/--exclude/--no-default-excludes/--title/--cwd/--help` + format, template, language, and working-tree flags).
