...
| FR-23 | Configurable fallback provider: `--fallback-provider` / `--fallback-endpoint` / `--fallback-model` retry generation with a secondary model config when the primary **errors** or returns a likely-invalid response. Unset fields inherit the primary's; tried before the deterministic changelog | **Shipped** | `releaseNotes.ts` (`hasFallback`, `runFallback`, `generateViaAI`); `ReleaseNotesOptions.fallback*` + `warn`. CLI in `cliArgs.ts`. Spec in [6-fallback.md](6-fallback.md). |
