...
| FR-15 | Apple Foundation Models provider: `--provider apple` for on-device, free, private generation on macOS 26+ (Apple Silicon + Apple Intelligence). A small Swift helper (`apple-fm-helper/main.swift`, built via `npm run build:apple-fm`) wraps `FoundationModels` and returns Markdown; `GITGIST_APPLE_FM_BIN` points at the binary. In `AUTO_ORDER` as a free fallback (a no-op when the helper isn't built) | **Shipped** | `providers/apple.ts` (`createAppleProvider`) spawns the helper (`--probe` / `--generate`, JSON stdin → Markdown stdout). |
| FR-16 | Releases ship a **Developer-ID-signed + notarized** prebuilt `apple-fm-helper` binary (arm64), built in a macOS CI job, so the `apple` provider works without a local Swift toolchain. Source build remains the fallback | **Shipped** | `release.yml` `apple-fm` job (sign + `notarytool` via app-specific password); binary force-included in package `files`; `apple.ts` resolves it relative to the package. Requires the Apple secrets (GG-19). |
