...
| FR-26 | Bounded, noise-filtered diff material: the patch body is capped at a char budget (default 24000, `--max-diff-chars`) and generated/lockfile/vendored paths are excluded from it, while the **complete changed-file list and stat are always kept**. Anything held back — a truncated patch, or files whose diff was dropped as noise — is stated in the prompt so the model knows the limit of what it may claim | **Shipped** | `git.ts` (`NOISE_PATHSPECS`, `capText`, `MAX_STAT_CHARS`, `DEFAULT_MAX_RANGE_DIFF_CHARS`); reported by `prompt.ts:rangeDiffToMaterial` via `RangeDiff.excluded`/`truncated`. Spec in [7-diff-grounding.md](7-diff-grounding.md). |
