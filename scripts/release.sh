...
# Interactive release flow for gitgist. Adapted from kerf / pglite-migrate /
# Hot Sheet — except the release-notes draft is generated by *gitgist itself*
# (it runs the built CLI over the commit range, dogfooding the tool).
#
# Usage:
...
#                                       version files, does NOT update the
#                                       changelog, does NOT commit; just tags
#                                       v{ver}-beta.{N} off HEAD; CI publishes
#                                       with --tag beta).
...
ask_multiline() {
  ...
  local tmpfile
  tmpfile=$(mktemp "${TMPDIR:-/tmp}/gitgist-release-notes.XXXXXX")
  ...
  while true; do
    ...
    # Strip guidance comments — lines that are a lone '#' or start with '# '
    # (hash + space) — then trailing blanks. This deliberately preserves
    # Markdown headings ('## Features', '### Bug Fixes') that gitgist emits.
    REPLY=$(grep -vE '^#( |$)' "$tmpfile" | sed -e :a -e '/^\n*$/{$d;N;ba' -e '}')
...
step_release_notes() {
  ...
  # Dogfood gitgist: generate the draft with the built CLI over the commit
  # range. Build first if the CLI isn't present yet.
  if [[ ! -f dist/cli.js ]]; then
    info "Building gitgist..."
    npm run build >/dev/null 2>&1 || true
  fi

  local generated=""
  if [[ -f dist/cli.js ]]; then
    info "Drafting release notes with gitgist${last_tag:+ (commits since ${last_tag})}..."
    if [[ -n "$last_tag" ]]; then
      generated=$(node dist/cli.js "${last_tag}..HEAD" 2>/dev/null || true)
    else
      # No tags yet → gitgist defaults to the full history.
      generated=$(node dist/cli.js 2>/dev/null || true)
    fi
    # gitgist already strips fences; trim any trailing blank lines defensively.
...
${generated}"
    ...
    warn "gitgist produced no draft — sign in to the claude CLI or set ANTHROPIC_API_KEY (or run gitgist --no-ai manually)."
    initial="# Release notes — keep them USER-FACING.
# Group changes under '## Section' headings; skip ticket IDs, refactors, tests,
# doc-only changes, and CI/build tweaks. Lines starting with '# ' are removed.
...
step_update_changelog() {
  ...
  # Demote gitgist's top-level section headings ('## Features') one level so
  # they nest under the changelog's '## [version]' entry.
  local body
  body=$(echo "$notes" | sed -E 's/^## /### /')

  local entry="## [${version}] - ${date}\n\n${body}"

  # Create a skeleton CHANGELOG if missing so the insert has a target.
...
HDR
  ...
  node -e "
    ...
    if (marker === -1) {
      // No versioned entry yet (first release): append below the header.
...
step_git_commit() {
  ...
  # Idempotent: if a previous run already absorbed these files into a commit
  # (e.g. recovery after a failed run), there's nothing left to stage. Skip
  # rather than fail under `set -e` so the tag-and-push step still runs.
...
step_stable_tag_and_push() {
  ...
  echo -e "    1. Run tests / lint / typecheck / build"
  echo -e "    2. Create a GitHub Release for ${tag}"
  echo -e "    3. Publish to npm with --provenance"
  echo ""
  echo -e "  ${DIM}Monitor:${RESET} https://github.com/brianwestphal/gitgist/actions"
}
...
step_beta_tag_and_push() {
  ...
  echo -e "  ${DIM}Users get it via:${RESET}  npm install gitgist@beta"
  echo ""
  echo -e "  ${DIM}Monitor:${RESET} https://github.com/brianwestphal/gitgist/actions"
}
...
main() {
  ...
  if [[ "$BETA_MODE" == "true" ]]; then
    echo -e "${BOLD}  gitgist Beta Release${RESET}"
    echo -e "  ${DIM}--beta mode: tag-only, no version-file bump.${RESET}"
  else
    echo -e "${BOLD}  gitgist Release${RESET}"
  fi
  echo ""
