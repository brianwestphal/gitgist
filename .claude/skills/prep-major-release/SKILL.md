...
```bash
node -p "require('./package.json').devDependencies['domotion-svg']"  # what's pinned
npm view domotion-svg version                                        # latest published
```

If the latest is newer than the pinned range, upgrade it and refresh the lockfile:

```bash
npm install --save-dev domotion-svg@latest   # bumps package.json + package-lock.json
```
...
- **A renderer upgrade means the visuals should be re-captured even if the demos
  ...
  Flag this in the Step 5 capture checklist so the maintainer re-runs `npm run demo`
...
- If domotion's CLI flags or output shape changed in the new version, the capture
  scripts (`scripts/demo.mjs`, `scripts/lib/terminal.mjs`, `scripts/diagram.mjs`)
...
- **Renderer upgrade**: whether `domotion-svg` was bumped (and to what version),
