...
diff** — added/modified/removed code, API/flag/provider/dep deltas — **not** commit messages
or the requirements docs (those describe the *end state* and the *whole* feature history, so
...
This is the same principle gitgist itself enforces (FR-25, [docs/7-diff-grounding.md](../../../docs/7-diff-grounding.md)):
the diff is the authority, prose is a lead to verify.
...
2. **The base is always the most recent production release tag** (e.g. `v1.1.0`), and the
...
2. **Run the analysis script** — it does the deterministic git work:
   ...
   HEAD, buckets the line delta **by area** (`src` core vs `src/providers` vs tests/scripts
   vs docs vs agent/skill scaffolding vs generated demo assets), gives a **product-only**
   total, lists **added/removed** files and candidate **new subsystems**, and extracts these
   gitgist-specific surface deltas:
   - **public API exports** — the name set of `src/index.ts` at each end (not raw diff lines);
   - **CLI flags** — the `case '--flag':` labels the parser actually handles;
   - **providers** — `PROVIDERS` registry membership plus the `AUTO_ORDER` sequence;
   - **requirements** — new `FR-`/`NFR-`/`T-` rows and status transitions;
   - **dependencies** — `package.json` deps + devDeps.
...
3. **Read the real diffs — do not stop at the script.** The script tells you *where* to
   ...
   added **and removed within the same range** (nets to zero at HEAD — say so); posture
   like "no API key" / "works offline" that was **already true at the base** (baseline, not
   a change); a dependency bumped in **two hops** (report the full base→HEAD delta).

   **The requirements delta needs this treatment most.** A new `FR-N` row is a *claim in a
   Markdown table*, and rows get added, reworded, and re-scoped independently of code. Before
   writing up a requirement as shipped work, find the code that implements it and the test
   that covers it (`rg '@covers FR-N' tests/`). A status flip from **Deferred → Shipped** is
   the interesting case — confirm the implementing diff is inside this range and not earlier.

4. **Write the report** to `docs/technical-changelog/<base>-<next>.md` (the script prints
   the exact suggested path; `mkdir -p` the directory on first use). Keep it to ~one page.
   It must contain, in this spirit:
   ...
   - **Honest size:** the area-by-area split from the script, and the **product-only**
     +/- total called out separately from the raw total (which is inflated by docs,
     regenerated demo SVGs under `assets/`, and `.claude`/`.agents`/`.gemini`/`.hotsheet`
     scaffolding — never present the raw number as engineering effort).
   ...
   - **Per-change sections** for the genuine deltas, each carrying its **diff evidence**
     (new files, the export/flag/provider/dep delta, `0-hits → present` for behavior). Order
     by significance (a net-new subsystem or provider before a doc sync).
...
- **Diff over prose.** If a claim isn't backed by a file/line change you actually read,
  don't make it. Commit subjects, `docs/3-requirements.md` rows, and the AI summaries are
  leads to verify, not sources to quote.
- **Never inflate.** Lead with product-only line counts; label docs, generated demo assets,
  and agent/skill scaffolding as non-engineering.
